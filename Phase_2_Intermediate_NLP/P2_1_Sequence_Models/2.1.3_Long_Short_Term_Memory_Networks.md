...
$\begin{align*} f_t &= \sigma \big(W_f \cdot [h_{t-1}, x_t] + b_f \big) \\ i_t &= \sigma \big(W_i \cdot [h_{t-1}, x_t] + b_i \big) \\ \tilde{C}_t &= \tanh \big(W_c \cdot [h_{t-1}, x_t] + b_c \big) \\ C_t &= f_t * C_{t-1} + i_t * \tilde{C}_t \\ o_t &= \sigma \big(W_o \cdot [h_{t-1}, x_t] + b_o \big) \\ h_t &= o_t * \tanh(C_t) \end{align*}$
